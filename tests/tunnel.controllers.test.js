...
describe('Tunnel Controllers', () => {
  ...
  describe('POST /api/v1/tunnels/startTunnel/:tunnelId?', () => {
    test('devrait démarrer tous les tunnels', async () => {
      ...
      const response = await request(app)
        .post('/api/v1/tunnels/startTunnel');

      expect(response.status).toBe(200);
    });
  });

  describe('POST /api/v1/tunnels/stopTunnel/:tunnelId?', () => {
    test('devrait arrêter un tunnel spécifique', async () => {
      fs.readdirSync.mockReturnValue(['tunnel1.pid']);
      
      const response = await request(app)
        .post('/api/v1/tunnels/stopTunnel/tunnel1');

      expect(response.status).toBe(200);
    });
  });

  describe('GET /api/v1/tunnels/status/:tunnelId?', () => {
  ...
  describe('POST /api/v1/tunnels/setBandwidth/:tunnelId', () => {
    test('devrait modifier la bande passante', async () => {
      readTunnelsConfig.mockReturnValue({ bandwidth: {} });
      
      const response = await request(app)
        .post('/api/v1/tunnels/setBandwidth/tunnel1')
